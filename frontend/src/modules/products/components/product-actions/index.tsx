...
export default function ProductActions({
  ...
  return (
    <div className="flex flex-col gap-3.5" ref={actionsRef}>
      ...
      {/* Quantity Stepper (on its own labeled row) */}
      {forSale && (
      ...
      {forSale && (
        <div className="small:hidden fixed bottom-0 left-0 right-0 z-50 bg-bg/95 backdrop-blur-md border-t border-line/60 px-4 py-2.5 flex items-center justify-between gap-4 pb-[safe-area-inset-bottom] shadow-[0_-8px_30px_rgba(0,0,0,0.08)]">
          <a
            href="#reviews"
            className="flex flex-col items-center justify-center gap-0.5 text-[10.5px] font-semibold text-ink/75 hover:text-primary transition-colors shrink-0 px-3 py-1 bg-surface rounded-full border border-line/40 hover:border-primary/40 active:scale-95 transition-all duration-200"
          >
            {reviewStats && reviewStats.reviewCount > 0 ? (
              <div className="flex items-center gap-1">
                <i className="ph-fill ph-star text-amber-500 text-[14px]" />
                <span className="text-xs font-bold text-ink leading-none">
                  {reviewStats.averageRating.toFixed(1)}
                </span>
                <span className="text-[10px] text-ink/45 font-medium leading-none">
                  ({reviewStats.reviewCount})
                </span>
              </div>
            ) : (
              <div className="flex items-center gap-1">
                <i className="ph ph-chat-circle-dots text-[15px]" />
          ...
          <button
            ...
            className="flex-1 h-11 px-6 rounded-full bg-primary text-primary-fg text-xs font-bold tracking-wide flex items-center justify-center gap-2 transition-all duration-200 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_4px_14px_-6px_rgb(var(--color-primary)/0.4)] hover:brightness-110"
          >
            {isAdding ? (
              <>
                <i className="ph-bold ph-spinner animate-spin text-sm" aria-hidden />
