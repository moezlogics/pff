...
const ShopFilters = ({
  ...
  const currentCategoryObj = useMemo(() => {
    ...
    return (categories || []).find((c: any) => c.handle === currentCategory)
  }, [categories, currentCategory])

  const categoriesToDisplay = useMemo(() => {
    if (!currentCategory) {
      // On main store page, show all top-level categories
      return (categories || []).filter((c: any) => !c.parent_category_id)
    }
    // On a single category page, show its direct children (subcategories)
    return currentCategoryObj?.category_children || []
  }, [categories, currentCategory, currentCategoryObj])
  ...
  return (
    ...
    >
      ...
      {categoriesToDisplay.length > 0 && (
        <AccordionItem title={currentCategory ? "Subcategories" : "Categories"} icon="ph-squares-four" activeCount={activeCategoriesCount} defaultOpen={true}>
          <ul className="flex flex-col gap-1">
            {!currentCategory ? (
              <li>
                <LocalizedClientLink
                  href="/store"
                  className="block px-2 py-1.5 rounded-base transition-colors bg-ink text-bg font-medium text-[13px]"
                >
                  All products
                </LocalizedClientLink>
              </li>
              ...
              <li>
                <LocalizedClientLink
                  href="/store"
                  className="block px-2 py-1.5 rounded-base transition-colors text-ink/80 hover:bg-surface font-semibold flex items-center gap-1.5 text-[13px]"
                >
                  <i className="ph ph-arrow-left text-[11px]" />
                  Back to All Shop
                </LocalizedClientLink>
              </li>
            )}
