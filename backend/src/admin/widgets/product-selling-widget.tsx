...
async function fetchForSale(productId: string): Promise<{
  ...
  // Default ON — if not explicitly set to false (or "false"), it is sellable.
  const forSale = m.for_sale !== false && m.for_sale !== "false"
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          ...
          <span style={{ fontSize: 11, color: "#6b7280" }}>
            {loading
              ? "Loading…"
              : forSale
                ? "Buy / Add-to-Cart buttons are SHOWN (default)."
                : "Buy / Add-to-Cart buttons are HIDDEN."}
          </span>
        </div>
        <Switch
          id="for-sale-switch"
